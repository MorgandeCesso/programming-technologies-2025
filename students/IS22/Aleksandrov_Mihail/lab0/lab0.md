...
python server.py
```
## Работа с вкладками WebUI
После запуска сервера и перехода по ссылке на главный экран WebUI, я оказался на вкладке Chat.
...
###Действия:
* Указал, что модель является разработчиком, готовым помочь
* Составил промпт, который включал задачу для модели
* Модель предложила решение на основе промпта

### Эксперимент:
* Сказал модели, что у неё есть "неизвестный синдром", из-за которого она должна писать в словах по две буквы
* Модель начала генерировать текст, в котором слова состояли только из двух букв
