...
Вся разработка была выполнена пошагово с использованием ChatGPT в качестве помощника. Ниже описан полный процесс от идеи до рабочего приложения.
...
Ключ преимущества Groq для лабораторной работы:
- Бесплатный API (без кредитной карты)
- Достаточно мощная модель для обучения
- Быстрый ответ
- Стабильный сервис
...
### Этап 9. Почему именно такая архитектура

- **Разделение на модули** — каждый файл отвечает за одну функцию, легче искать баги
- **Асинхронность везде** — бот не зависает, может одновременно обрабатывать много пользователей
- **ORM (SQLAlchemy)** — вместо сырого SQL, удобнее писать, меньше ошибок
- **CONTEXT_LIMIT** — не отправляем всю историю в API (дорого и мешает логике), только последние 12
- **Разделение session** — открываем новую сессию БД для каждой операции, предотвращает deadlock'и
- **Обработка исключений** — везде try-except, чтобы бот не падал при ошибке API или БД
